...
@app.on_message(Filters.command(['stats', 'stats@NewRotomonBot']))
...
# ===== Home =====
@app.on_message(Filters.command(['start', 'start@NewRotomonBot']))
...
@app.on_message(Filters.command(['type', 'type@NewRotomonBot']))
...
def ptype_buttons(user_id):
    keyboard = ([[
        ...
        InlineKeyboardButton(
            'Fighting', callback_data=f"type_fighting_{user_id}"),
    ...
    keyboard += ([[
        InlineKeyboardButton(
            'Electric', callback_data=f"type_electric_{user_id}"),
        InlineKeyboardButton(
            'Psychic', callback_data=f"type_psychic_{user_id}"),
...
@app.on_message(Filters.command(['types', 'types@NewRotomonBot']))
...
@app.on_message(Filters.command(['ptype', 'ptype@NewRotomonBot']))
def poketypes(app, message):
    ...
    try:
        ...
        keyboard = ([[
            InlineKeyboardButton(
                p_type['type1'], callback_data=f"poket_{p_type['type1']}_{arg}_{user_id}"),
...
def poketypes_back(client: app, callback_query: CallbackQuery):
    ...
    if callback_query.from_user.id == user_id:
        ...
        try:
            ...
            keyboard = ([[
                InlineKeyboardButton(
                    p_type['type1'], callback_data=f"poket_{p_type['type1']}_{query_data}_{user_id}"),
...
@app.on_message(Filters.command(['data', 'data@NewRotomonBot']))
def pkmn_search(app, message):
    try:
        if message.text == '/data' or message.text == '/data@NewRotomonBot':
            app.send_message(
                message.chat.id, texts['error1'], parse_mode='HTML')
            return None
...
@app.on_message(Filters.command(['usage', 'usage@NewRotomonBot']))
...
# ===== FAQ command =====
@app.on_message(Filters.command(['faq', 'faq@NewRotomonBot']))
def faq(app, message):
    text = texts['faq']
    app.send_message(
        chat_id=message.chat.id,
...
# ===== About command =====
@app.on_message(Filters.command(['about', 'about@NewRotomonBot']))
...
# ===== Raid commands =====
@app.on_message(Filters.command(['addcode', 'addcode@NewRotomonBot']))
def call_add_fc(app, message):
    raid.add_fc(app, message, texts)


@app.on_message(Filters.command(['mycode', 'mycode@NewRotomonBot']))
def call_show_my_fc(app, message):
    raid.show_my_fc(app, message, texts)


@app.on_message(Filters.command(['newraid', 'newraid@NewRotomonBot']))
